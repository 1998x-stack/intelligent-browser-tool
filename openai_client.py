"""
LLM客户端模块 - Ollama API封装 (使用OpenAI包)

设计理念 (CleanRL哲学):
- 单文件自包含: 所有LLM交互逻辑集中管理
- 透明的处理流程: 请求/响应流程清晰
- 最小化抽象: 使用OpenAI包调用Ollama
- 便于调试: 详细的请求日志
...
注意: Ollama 提供 OpenAI 兼容的 API，所以可以使用 openai 包调用
"""
...
class LLMConfig:
    """LLM配置 (Ollama)"""
    base_url: str = "http://localhost:11434/v1"  # Ollama OpenAI兼容端点
...
class LLMClient:
    """
    LLM客户端 - 使用OpenAI包封装Ollama API调用
    ...
    def __init__(self, config: LLMConfig):
        ...
        # 初始化OpenAI客户端，指向Ollama服务
        # Ollama不需要真实的API key，但OpenAI包要求提供
        self.client = OpenAI(
            base_url=config.base_url,
            api_key="ollama",  # Ollama不验证API key
            timeout=config.timeout,
            max_retries=config.max_retries,
        )
        
        logger.info(f"LLM客户端初始化完成 (URL: {config.base_url})")
    ...
    def generate(
        ...
        **kwargs
        ...
        Args:
            ...
            temperature: 温度参数
            max_tokens: 最大token数
            stream: 是否流式输出（注意：此方法不支持流式，请使用generate_stream）
            **kwargs: 其他参数
...
if __name__ == "__main__":
    print("=" * 60)
    print("LLM客户端模块测试 (Ollama + OpenAI包)")
    print("=" * 60)
    ...
    for chunk in client.generate_stream("用一句话解释人工智能"):

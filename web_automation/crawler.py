...
class WebCrawler:
    ...
    def __init__(self, crawl_config: CrawlConfig, config: Optional[Config] = None):
        ...
        logger.info(
            f"WebCrawler initialized: start_url={crawl_config.start_url}, "
            f"intent={crawl_config.intent}, max_pages={crawl_config.max_pages}"
        )
    ...
    def _init_components(self) -> None:
        ...
        try:
            ...
            self.url_queue = URLQueue(
                ...
                max_size=self.crawl_config.max_pages * 10,
                allowed_domains=[start_domain] if start_domain else None,
                exclude_patterns=self.config.browser.exclude_patterns
            )
            
            # 存储管理器
            self.storage = StorageManager(
                base_dir=self.crawl_config.output_dir,
    ...
    def _crawl_loop(self) -> None:
        ...
        while not self.url_queue.is_empty() and pages_processed < self.crawl_config.max_pages:
            # 获取下一个URL
    ...
    def _process_page(self, item: QueueItem) -> PageResult:
        ...
        try:
            ...
            if self.crawl_config.save_raw_html:
                html_filename = self.file_namer.generate_unique_name(
                    url=item.url,
                    title="",
                    content_type="html",
                    output_dir=str(self.storage.raw_dir)
                )
            ...
            analysis = self.content_analyzer.analyze(
                ...
                url=item.url,
                links=[(link.url, link.text) for link in extracted.links[:50]],
                intent_components=self.intent_components
            )
    ...
    def _save_page_result(self, result: PageResult, extracted) -> None:
        ...
        try:
            # 生成文件名
            filename = self.file_namer.generate_unique_name(
                url=result.url,
                title=result.title,
                content_type="analysis",
                output_dir=str(self.storage.processed_dir)
            )
